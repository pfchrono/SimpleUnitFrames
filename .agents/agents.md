...
- Prefer `SUF:` namespaced functions with locals declared near the top of each file; keep style compact and low-comment like existing code in [Core/UnitFrame.lua](../Core/UnitFrame.lua) and [Elements/CastBar.lua](../Elements/CastBar.lua).
...
- **Change detection patterns:**
  - Use `SUF:StampChanged(obj, key, value)` to avoid redundant updates (see [Core/Helpers.lua](../Core/Helpers.lua))
  - Use `SUF:SetPointIfChanged(frame, ...)` instead of unconditional `SetPoint()` (14+ indicator files use this pattern)
  - Frame config caching: Store `frame.SUFUnitConfig = UnitDB` during creation to avoid repeated lookups
- **Frame storage patterns:**
  - Use dual storage: `SUF[unit:upper()]` (legacy, e.g., SUF.PLAYER) AND `SUF.Units[unit]` (modern, e.g., SUF.Units["player"])
  - SUF.Units table initialized in [Core/Globals.lua](../Core/Globals.lua), populated in [Core/UnitFrame.lua](../Core/UnitFrame.lua)
  - Both patterns maintained for backward compatibility and Phase 5 optimization systems
  - Boss frames: SUF.Units["boss1"] through SUF.Units["boss5"]
  - Party frames: SUF.Units["party1"] through SUF.Units["party5"] (or "player")
  - Single frames: SUF.Units["player"], SUF.Units["target"], SUF.Units["pet"], etc.
- **Utilities module:** Use `SUF.Utilities` helpers for common operations (see [Core/Utilities.lua](../Core/Utilities.lua)):
...
- **Debug output:** Use `SUF.DebugOutput` for all diagnostic messages instead of print() (see [Core/DebugOutput.lua](../Core/DebugOutput.lua)):
  - API: `SUF.DebugOutput:Output(systemName, message, tier)` where tier is TIER_CRITICAL/TIER_INFO/TIER_DEBUG
  - Three tiers: Critical (chat + panel), Info (panel optional), Debug (system-specific)
  - System-specific: Check `SUF.db.global.Debug.systems.SystemName` before debug output
  - Tier constants: `SUF.DebugOutput.TIER_CRITICAL` (1), `SUF.DebugOutput.TIER_INFO` (2), `SUF.DebugOutput.TIER_DEBUG` (3)
  - Example: `SUF.DebugOutput:Output("Validator", "Check passed", SUF.DebugOutput.TIER_INFO)`
  - Access panel: `/SUFdebug` command
  - Export logs: Click "Export to Clipboard" in debug panel

## Architecture
- AddOn load order: libraries → elements → core via [SimpleUnitFrames.toc](../SimpleUnitFrames.toc), [Elements/Init.xml](../Elements/Init.xml), and [Core/Init.xml](../Core/Init.xml).
...
- Initialization and event wiring are in [Core/Core.lua](../Core/Core.lua); global utilities, media registration, and `SUF:ResolveLSM()` live in [Core/Globals.lua](../Core/Globals.lua).
...
- **Architecture module** ([Core/Architecture.lua](../Core/Architecture.lua)) provides:
  - **EventBus:** Singleton event dispatcher (`SUF.Architecture.EventBus`) with Register/Unregister/Dispatch API
...
- **Performance Systems** (Phase 4a-4c + Phase 5 Priority 1, 45-85% total improvement, ZERO HIGH frame spikes):
  ...
  - **DebugOutput & DebugPanel** ([Core/DebugOutput.lua](../Core/DebugOutput.lua), [Core/DebugPanel.lua](../Core/DebugPanel.lua)): 3-tier output (Critical/Info/Debug), system-specific enable flags, `/SUFdebug` command, export to clipboard
...
- **CastBar Enhancements** ([Elements/CastBarEnhancements.lua](../Elements/CastBarEnhancements.lua)): Visual indicators for casting
  ...
  - **Channel Ticks:** Visual tick markers for channel ability timing (uses `SUF:GetChannelTicks(spellID)` from ChannelingTicks table)
  ...
  - **Initialization:** `SUF:EnhanceCastBar()` creates elements on cast start, `SUF:UpdateCastBarEnhancements()` updates per-frame
  - **ChannelingTicks:** `SUF.ChannelingTicks` table maps spell IDs to tick arrays (milliseconds); `SUF:GetChannelTicks(spellID)` returns ticks with fallback to DEFAULT

## Build and Test
- No build/test commands are documented in this repo.

## Project Conventions
- Use `SUF:QueueOrRun` for protected operations during combat lockdown (see [Core/Core.lua](../Core/Core.lua)).
...
- Media is resolved through `SUF.Media` populated by `SUF:ResolveLSM()` in [Core/Globals.lua](../Core/Globals.lua).
...
- **Change detection best practices:**
  ...
  - Cache frame configs during creation (`frame.SUFUnitConfig = UnitDB`) to avoid repeated DB lookups
...
- **Performance monitoring:**
  - Use `/SUFperf` to toggle real-time performance dashboard
  - Use `/SUFprofile start|stop|analyze|export` for timeline profiling
  - Use `/SUFpreset low|medium|high|ultra` to adjust performance levels
  - Use `/run SUF.Validator:RunFullValidation()` to check system health
  - Use `/run SUF.PerformanceDashboard:PrintStats()` for comprehensive stats to chat
  - Use `/run SUF.FrameTimeBudget:PrintStatistics()` for frame time budget stats
  - Use `/SUFdebug` to open debug panel for system-specific diagnostics
...
- **Machine Learning Optimization:**
  ...
  - Check recommendations: `/run SUF.DirtyPriorityOptimizer:PrintRecommendations()`
- **Adaptive batching:**
...
- **Conditional frame handling:**
  ...
  - Validation: Check frame exists (SUF[unit:upper()]) but don't require IsVisible() for conditional frames
  - Both storage patterns available: SUF.PLAYER and SUF.Units["player"]
...
- **New Architecture Systems:**
  ...
  - PerformanceDashboard (`/SUFperf` for real-time metrics, PrintStats() for chat output)
  - DebugOutput & DebugPanel (non-intrusive debug routing with `/SUFdebug` UI)
...
## Security
- Combat lockdown applies to frame changes; defer protected changes using `SUF:QueueOrRun` and avoid in-combat layout edits (see [Core/Core.lua](../Core/Core.lua)).
...
- **Debug Output Safety:**
  ...
  - System-specific enable flags (SUF.db.global.Debug.systems.SystemName) prevent spam
