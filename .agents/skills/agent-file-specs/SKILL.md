...
> **Important:** Detailed specifications live in the `references/` folder â€”
...
| **Skill** | `SKILL.md` (directory-based) | Reusable, portable capabilities with scripts and resources â€” follows the Agent Skills open standard | [SKILLS.md](references/SKILLS.md) |
...
| **File Locations** | â€” | Provider folder mapping, directory structure, naming conventions | [FILE-LOCATIONS.md](references/FILE-LOCATIONS.md) |
| **VS Code Settings** | â€” | Settings that control customization file discovery and behavior | [SETTINGS.md](references/SETTINGS.md) |
...
- **"I want coding standards applied everywhere"** â†’ Use always-on instructions
  (`.github/copilot-instructions.md` or `AGENTS.md`).
  See [INSTRUCTIONS.md](references/INSTRUCTIONS.md).

- **"I want different rules for different file types"** â†’ Use file-based
  instructions (`.instructions.md` with `applyTo` patterns).
  See [INSTRUCTIONS.md](references/INSTRUCTIONS.md).

- **"I have a reusable task I run repeatedly"** â†’ Use a prompt file
  (`.prompt.md`) invoked as a slash command.
  See [PROMPTS.md](references/PROMPTS.md).

- **"I need a multi-step workflow with scripts and resources"** â†’ Use an Agent
  Skill (directory with `SKILL.md`). Portable across tools.
  See [SKILLS.md](references/SKILLS.md).

- **"I need a specialized AI persona with tool restrictions"** â†’ Use a custom
