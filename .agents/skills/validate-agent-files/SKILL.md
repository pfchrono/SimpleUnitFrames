...
- `<provider>/agents/*.md` â†’ Agent file (user-invokable)
- `<provider>/agents/*.subagent.agent.md` â†’ Sub-agent file (workflow component)
- `<provider>/skills/*/SKILL.md` â†’ Skill file
- `<provider>/prompts/*.prompt.md` â†’ Prompt file
- `<provider>/instructions/*.instructions.md` â†’ Instruction file
...
**Checks:**
1. âœ“ YAML frontmatter present with `---` delimiters
2. âœ“ `name` field exists and is non-empty
3. âœ“ `description` field exists (recommend 50+ characters with examples)
4. âœ“ Body content exists after frontmatter
5. âœ“ If `tools` specified, they are valid tool names
6. âœ“ If filename contains `.subagent.agent.md`, verify `user-invokable: false` is set
...
**Checks:**
1. âœ“ File is named `SKILL.md` inside a directory
2. âœ“ `name` matches parent directory name exactly
3. âœ“ `name` is lowercase, alphanumeric with hyphens only
4. âœ“ `name` doesn't start/end with hyphen or have consecutive hyphens
5. âœ“ `description` is 1-1024 characters
6. âœ“ Body content provides clear instructions
...
**Checks:**
1. âœ“ File has `.prompt.md` extension
2. âœ“ If `mode` present, it's a valid value
3. âœ“ Variables use `{{variableName}}` syntax
4. âœ“ Body content exists (the prompt itself)
...
**Checks:**
1. âœ“ File has `.instructions.md` extension
2. âœ“ `applyTo` field exists
3. âœ“ `applyTo` contains valid glob pattern(s)
4. âœ“ Body content provides meaningful guidance
...
**Status:** âœ… Valid | âš ï¸ Warnings | âŒ Invalid
