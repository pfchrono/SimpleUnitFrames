...
# Achievements API (Retail â€” Patch 12.0.0)
...
- **Achievement Functions** â€” GetAchievementInfo, criteria, categories
- **Statistics** â€” GetStatistic, comparison
- **Tracking** â€” Tracked achievements
- **Guild Achievements** â€” Guild-specific achievements
...
| `ClearAchievementComparisonUnit()` | â€” | Clear comparison |
...
| `AddTrackedAchievement(achievementID)` | â€” | Track achievement |
| `RemoveTrackedAchievement(achievementID)` | â€” | Untrack achievement |
...
| `CRITERIA_UPDATE` | â€” | Criteria progress updated |
| `CRITERIA_COMPLETE` | â€” | All criteria complete |
...
| `ACHIEVEMENT_SEARCH_UPDATED` | â€” | Search results ready |
...
1. **GetAchievementInfo returns 15 values** â€” Destructure carefully. The `completed` boolean is the 4th return.
2. **Guild vs personal** â€” Pass `true` as `isGuild` to `GetCategoryNumAchievements()` for guild achievements.
3. **Statistics are strings** â€” `GetStatistic()` returns a formatted string, not a number. Parse with `tonumber()` if needed.
4. **Criteria index is 1-based** â€” Criteria indices start at 1, up to `GetAchievementNumCriteria()`.
5. **Achievement chains** â€” Some achievements are chained (10/25/50 kills etc.). Use `GetPreviousAchievement()`/`GetNextAchievement()`.
6. **Search is async** â€” `SetAchievementSearchString()` may not return all results immediately. Wait for `ACHIEVEMENT_SEARCH_UPDATED`.
7. **Tracking limit** â€” There's a maximum number of tracked achievements.
8. **Inspect data** â€” Achievement comparison requires `SetAchievementComparisonUnit()` and waiting for `INSPECT_ACHIEVEMENT_READY`.
