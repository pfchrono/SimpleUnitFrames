...
# Combat API (Retail â€” Patch 12.0.0)
...
- **Combat Log** â€” New 12.0.0 combat log system, COMBAT_LOG_MESSAGE
- **C_DamageMeter** â€” Built-in damage/healing meter system
- **Threat** â€” UnitThreatSituation, UnitDetailedThreatSituation
- **C_LossOfControl** â€” Loss of control tracking
- **C_CombatText** â€” Floating combat text
- **C_CombatAudioAlert** â€” Combat audio alert system (accessibility)
- **C_Secrets / C_CurveUtil / C_DurationUtil** â€” Secret value handling for combat data
- **Encounter Events** â€” ENCOUNTER_STATE_CHANGED, ENCOUNTER_WARNING, timeline events
- **Spectator/Commentator** â€” C_Commentator for spectator mode

---

## C_DamageMeter â€” Built-in Damage Meter
...
| `C_DamageMeter.ResetSessions()` | â€” | Clear all sessions |
...
- `locType` â€” Type of CC (STUN, FEAR, SILENCE, etc.)
- `spellID` â€” Spell causing the CC
- `displayText` â€” Localized display text
- `iconTexture` â€” Spell icon
- `startTime` â€” When CC started
- `timeRemaining` â€” Duration remaining (may be DurationObject in 12.0.0)
- `duration` â€” Total duration
- `lockoutSchool` â€” School lockout (for SCHOOL_INTERRUPT type)
- `priority` â€” Display priority
...
## C_CombatAudioAlert â€” Combat Audio Alerts
...
| `C_CombatAudioAlert.SetEnabled(enabled)` | â€” | Enable/disable |

---

## Secret Value Handling for Combat Data

### C_Secrets â€” Secret Predicates
...
### C_CurveUtil â€” Curves for Secret Values
...
```lua
-- Create a color curve: green at 100% â†’ red at 0%
local colorCurve = C_CurveUtil.CreateColorCurve()
-- Configure curve points (green â†’ yellow â†’ red)
...
### C_DurationUtil â€” Duration Objects
...
| `C_Commentator.StartWargame(...)` | â€” | Start a wargame |
| `C_Commentator.UpdatePlayerInfo()` | â€” | Refresh player data |
...
if InCombatLockdown() then
    print("In combat lockdown â€” restricted operations blocked")
end
```
...
1. **CLEU is GONE** â€” Do NOT register for `COMBAT_LOG_EVENT_UNFILTERED`. It will not fire for addon code in 12.0.0.
2. **No combat log parsing** â€” `CombatLogGetCurrentEventInfo()` is unavailable to tainted code.
3. **KString chat messages** â€” Combat log chat tab output is now KStrings that cannot be parsed.
4. **Health is secret** â€” `UnitHealth()` returns secret values in instances. Pass directly to `StatusBar:SetValue()`.
5. **Use DurationObjects** â€” Don't do manual cooldown math. Use `C_DurationUtil.CreateDuration()` and `Cooldown:SetCooldownFromDurationObject()`.
6. **Built-in damage meter** â€” Use `C_DamageMeter` for damage/healing data instead of parsing combat events.
7. **SendAddonMessage blocked** â€” Cannot send addon messages in instances. Boss mod addons must use new encounter event patterns.
8. **Unit identity restricted** â€” `UnitName()`, `UnitClass()` may return secrets for enemy units in instances during combat.
9. **Threat in instances** â€” Threat values may be secret. Pass to widgets directly; don't branch on them.
10. **Design philosophy** â€” Addons should NOT provide competitive advantage in combat. Blizzard provides built-in replacements for most restricted functionality.
