...
# Encounters API (Retail â€” Patch 12.0.0)
...
- **Encounter Journal** â€” Boss database, loot tables, abilities
- **C_ChallengeMode** â€” M+ keystones, affixes, timers
- **C_MythicPlus** â€” Weekly M+ runs, rating, seasonal
- **ScenarioInfo** â€” Scenarios, delves, stages
- **InstanceLeaverInfo** â€” Deserter/leaver info
- **Raid Locks** â€” Saved instance lockouts
- **C_SeasonInfo** â€” Seasonal content
...
| `EJ_SelectTier(tierIndex)` | â€” | Select expansion tier |
...
| `EJ_GetNumInstances()` | â€” | Not a function; iterate `EJ_GetInstanceByIndex` until nil |
| `EJ_SelectInstance(instanceID)` | â€” | Select instance |
...
| `EJ_SelectEncounter(encounterID)` | â€” | Select encounter |
...
- `description` â€” Section text
- `headerType` â€” Header type enum
- `abilityIcon` â€” Icon texture
- `creatureDisplayID` â€” Display ID
- `siblingSectionID` â€” Next sibling
- `firstChildSectionID` â€” First child
- `filteredByDifficulty` â€” Hidden by difficulty filter?
- `link` â€” Journal link
- `startsOpen` â€” Default expanded?
...
| `EJ_SetLootFilter(classID, specID)` | â€” | Filter by class/spec |
| `EJ_GetLootFilter()` | `classID, specID` | Current filter |
| `EJ_SetSlotFilter(slotFilter)` | â€” | Filter by slot |
| `EJ_GetSlotFilter()` | `slotFilter` | Current slot filter |
| `EJ_ResetLootFilter()` | â€” | Clear all filters |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `EJ_SetDifficulty(difficultyID)` | â€” | Set EJ difficulty |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `EJ_SetSearch(text)` | â€” | Set search text |
...
| `EJ_ClearSearch()` | â€” | Clear search |

---

## C_ChallengeMode â€” Mythic+ Keystones
...
## C_MythicPlus â€” Seasonal M+ Data
...
| `C_MythicPlus.RequestCurrentAffixes()` | â€” | Request affix data |
| `C_MythicPlus.RequestMapInfo()` | â€” | Request map data |
| `C_MythicPlus.RequestRewards()` | â€” | Request reward data |
| `C_MythicPlus.IsDungeonScorable(mapID)` | `isScorable` | Awards score? |

---

## ScenarioInfo â€” Scenarios & Delves
...
- `name` â€” Scenario name
- `currentStage` â€” Current stage
- `numStages` â€” Total stages
- `flags` â€” Scenario flags
- `isComplete` â€” Completed?
- `xp` â€” XP reward
- `money` â€” Gold reward
- `type` â€” Scenario type
- `area` â€” Area name
- `uiTextureKit` â€” UI texture kit

### Step Info Fields

- `title` â€” Step title
- `description` â€” Step description
- `numCriteria` â€” Criteria count
- `isComplete` â€” Step complete?
- `isBonusObjective` â€” Bonus objective?
- `isFinal` â€” Final step?
- `weightedProgress` â€” Progress percentage
...
| `SetSavedInstanceExtend(index, extend)` | â€” | Extend lockout |
| `GetNumSavedWorldBosses()` | `count` | World boss locks |
| `GetSavedWorldBossInfo(index)` | `name, id, reset` | World boss info |
| `RequestRaidInfo()` | â€” | Request lock data |

---

## InstanceLeaverInfo â€” Deserter
...
| `CHALLENGE_MODE_DEATH_COUNT_UPDATED` | â€” | Death count changed |
| `CHALLENGE_MODE_KEYSTONE_RECEPTABLE_OPEN` | â€” | Font of Power opened |
| `CHALLENGE_MODE_KEYSTONE_SLOTTED` | â€” | Keystone slotted |
| `MYTHIC_PLUS_CURRENT_AFFIX_UPDATE` | â€” | Affixes updated |
| `MYTHIC_PLUS_NEW_WEEKLY_RECORD` | mapID | New weekly best |
| `MYTHIC_PLUS_NEW_SEASON_RECORD` | mapID | New season best |
| `SCENARIO_UPDATE` | â€” | Scenario state changed |
| `SCENARIO_CRITERIA_UPDATE` | â€” | Criteria updated |
| `SCENARIO_COMPLETED` | â€” | Scenario completed |
| `UPDATE_INSTANCE_INFO` | â€” | Saved instance data |
| `INSTANCE_LOCK_START` | â€” | Lockout acquired |
| `INSTANCE_LOCK_STOP` | â€” | Lockout released |
| `INSTANCE_LOCK_WARNING` | â€” | Lockout warning |

---

## Gotchas & Restrictions

1. **EJ data is async** â€” Loot data loads asynchronously. Wait for `EJ_LOOT_DATA_RECIEVED` before reading loot.
2. **Tier selection matters** â€” EJ functions operate on the currently selected tier. Always set tier first.
3. **M+ completion data is temporary** â€” `GetCompletionInfo()` only returns data immediately after a run.
4. **Keystone slotting requires hardware event** â€” Players must manually interact with the Font of Power.
5. **Raid lock extensions** â€” `SetSavedInstanceExtend()` must be called during the lock's active period.
6. **Scenario types vary** â€” Delves, island expeditions, and scenarios all use ScenarioInfo with different type flags.
7. **M+ rating is per-season** â€” `GetOverallDungeonScore()` resets each season.
8. **RequestRaidInfo() is async** â€” Wait for `UPDATE_INSTANCE_INFO` before reading saved instances.
