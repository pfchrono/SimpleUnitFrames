...
- `AA` â€” Alpha (currently ignored; always use `FF`).
- `RR` `GG` `BB` â€” Red, green, blue channels as two hex digits each.
- `|r` â€” Pops the most recent color, restoring the previous one (since Patch 9.0.1). Previously it reset to the default color entirely.
...
Renders `text` using the color for item quality `n`, where `n` is a numeric `Enum.ItemQuality` value (0 = Poor/gray, 1 = Common/white, 2 = Uncommon/green, 3 = Rare/blue, 4 = Epic/purple, 5 = Legendary/orange, â€¦). The color updates to match the user's color-override settings.
...
| `height` | `width` | Result |
|-----------|---------|--------|
| `0` | omitted | Square icon at text line height (`TextHeight Ã— TextHeight`) |
| `> 0` | omitted | Square icon (`height Ã— height`) |
| `0` | `0` | Square at `TextHeight` |
| `> 0` | `0` | `TextHeight` wide, `height` tall |
| `0` | `> 0` | `width` is treated as an **aspect ratio**: `Width = width Ã— TextHeight`, `Height = TextHeight` |
| `> 0` | `> 0` | Explicit `width Ã— height` |
...
| `rVertexColor`, `gVertexColor`, `bVertexColor` | RGB tint values, each 0â€“255 |
...
-- Cropped texture (4px border removed from a 64Ã—64 source)
...
```lua
print("ë¼ë©´|1ì„;ë¥¼;")  -- ë¼ë©´ì„ (ramyeoneul)
print("ë‚˜ë¬´|1ì„;ë¥¼;")  -- ë‚˜ë¬´ë¥¼ (namureul)
```

### `|2` â€” French Prepositions
...
### `|3` â€” Russian Noun Declension

```
|3-id(text)
```

Declines a Russian noun into one of 5 cases (id = 1â€“5).

```lua
print("|3-1(ÐšÑ€Ð¾Ð»Ð¸Ðº)") -- ÐšÑ€Ð¾Ð»Ð¸ÐºÐ°   (Genitive)
print("|3-2(ÐšÑ€Ð¾Ð»Ð¸Ðº)") -- ÐšÑ€Ð¾Ð»Ð¸ÐºÑƒ   (Dative)
print("|3-3(ÐšÑ€Ð¾Ð»Ð¸Ðº)") -- ÐšÑ€Ð¾Ð»Ð¸ÐºÐ°   (Accusative)
print("|3-4(ÐšÑ€Ð¾Ð»Ð¸Ðº)") -- ÐšÑ€Ð¾Ð»Ð¸ÐºÐ¾Ð¼  (Instrumental)
print("|3-5(ÐšÑ€Ð¾Ð»Ð¸Ðº)") -- ÐšÑ€Ð¾Ð»Ð¸ÐºÐµ   (Prepositional)
```
...
### `|7` â€” Plural (Large Numbers / Non-English)
...
- [Hyperlinks](https://warcraft.wiki.gg/wiki/Hyperlinks) â€” related pipe-delimited link sequences (`|H...|h...|h`).
- `wow-api-framexml` skill â€” covers `WrapTextInColorCode`, `CreateTextureMarkup`, `CreateSimpleTextureMarkup`, `CreateAtlasMarkup` in detail.
- `wow-api-widget` skill â€” covers FontString, GameTooltip, and other widgets that render these sequences.
