...
# Lua Environment & Security (Retail â€” Patch 12.0.0)
...
- **Lua Sandbox** â€” WoW's Lua 5.1 environment, restricted standard library, blocked functions
- **Taint System** â€” How addon code becomes tainted and what tainted code cannot do
- **Secure Execution** â€” Protected functions, secure frames, secure handlers
- **Combat Lockdown** â€” What addons can and cannot do during combat
- **C_Timer** â€” Timer functions (After, NewTicker, NewTimer)
- **Hooks** â€” hooksecurefunc, securecallfunction, securecallmethod
- **C_RestrictedActions** â€” Addon restriction state queries
- **C_Log** â€” Logging utilities
- **FrameScript** â€” Frame script environment, secret values, scrubbing
- **Debugging** â€” Error handling, stack traces, debugging utilities
...
| `newproxy()` | Removed |
| `getfenv()` | Limited â€” returns read-only |
...
- `print()` â€” outputs to default chat frame
...
2. Tainted values **propagate** â€” if tainted data flows into Blizzard code, it taints that path
3. Protected functions check taint before executing â€” they fail if execution path is tainted
...
```lua
-- WRONG â€” This taints the Blizzard settings table
Settings.RegisterAddOnCategory = myFunc  -- TAINT!

-- WRONG â€” Modifying secure frame in insecure context
local btn = PlayerFrame  -- This is a secure Blizzard frame
btn:SetAttribute("type", "spell")  -- TAINT â€” can cause action blocked errors

-- RIGHT â€” Use hooksecurefunc for observation without tainting
hooksecurefunc("SomeBlizzardFunction", function(...)
    -- Your code runs AFTER the original â€” doesn't taint
end)
```
...
frame:SetScript("OnEvent", function()
    -- Combat ended â€” safe to modify secure frames now
...
```lua
-- SecureActionButtonTemplate â€” allows protected actions via user clicks
...
-- SecureHandlerBaseTemplate â€” run secure snippets
...
|----------|---------|-------------|
| `C_Timer.After(seconds, callback)` | â€” | One-shot timer |
...
## C_RestrictedActions â€” Addon Restriction State
...
| `forceinsecure()` | â€” | Force insecure execution |
| `seterrorhandler(handler)` | â€” | Set global error handler |
...
```lua
-- /dump expression â€” evaluates and prints
-- /run code â€” executes Lua code
-- /script code â€” same as /run
-- /console cvarName [value] â€” get/set console variables
```
...
frame:SetScript("OnEvent", function(self, event)
    -- Safe to initialize â€” player is logged in
...
if not issecretvalue(someValue) then
    ...
    -- Cannot inspect â€” pass to UI widget directly
...
1. **No `require()`** â€” WoW has no module system. Use the TOC file to control load order. Libraries are embedded directly.
2. **`setfenv()` / `getfenv()`** â€” Severely restricted. Do not rely on environment manipulation.
3. **`collectgarbage()`** â€” Only `"count"` mode works. Cannot force GC collection.
4. **Taint is sticky** â€” Once a variable is tainted, it stays tainted. Even if you set it back to the original value, the taint remains.
5. **`print()` goes to chat** â€” Unlike standard Lua, `print()` outputs to the default chat frame, not stdout.
6. **String library additions** â€” WoW adds `strsplit`, `strjoin`, `strtrim`, and `strmatch` as globals (in addition to `string.match`).
7. **No `os.exit()`** â€” Cannot terminate the client programmatically.
8. **Coroutines work** â€” Full coroutine support is available and commonly used for async patterns.
9. **Secret values (12.0.0)** â€” Some API returns are now opaque "secret" values that cannot be inspected, compared, or used in arithmetic. See the `wow-api-important` instructions for full details.
10. **Instance restrictions (12.0.0)** â€” `SendAddonMessage()` is blocked in instances. Design addons to work without inter-player communication during instanced content.
