...
# Professions API (Retail â€” Patch 12.0.0)
...
- **C_TradeSkillUI** â€” Recipe browsing, crafting, reagents, skill info
- **C_CraftingOrders** â€” Crafting order system (customer & crafter)
- **C_ProfessionSpecUI** â€” Profession specialization trees
- **Global TradeSKill** â€” Legacy/global tradeskill functions

---

## C_TradeSkillUI â€” Tradeskill System
...
| `C_TradeSkillUI.CloseTradeSkill()` | â€” | Close tradeskill UI |
...
| `C_TradeSkillUI.SetRecipeRepeatCount(count)` | â€” | Set repeat count |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_TradeSkillUI.CraftRecipe(recipeID [, count [, craftingReagents [, recipeLevel [, orderID]]]])` | â€” | Craft items |
| `C_TradeSkillUI.RecraftRecipe(itemGUID, craftingReagents)` | â€” | Recraft an item |
| `C_TradeSkillUI.CraftSalvage(recipeID, count, itemTarget)` | â€” | Salvage crafting |
| `C_TradeSkillUI.CraftEnchant(recipeID [, count [, craftingReagents]])` | â€” | Craft enchant |
...
| `C_TradeSkillUI.SetRecipeItemNameFilter(text)` | â€” | Filter by name |
| `C_TradeSkillUI.GetRecipeItemNameFilter()` | `text` | Current name filter |
| `C_TradeSkillUI.SetOnlyShowMakeableRecipes(onlyMakeable)` | â€” | Filter to makeable |
| `C_TradeSkillUI.GetOnlyShowMakeableRecipes()` | `onlyMakeable` | Showing only makeable? |
| `C_TradeSkillUI.SetOnlyShowSkillUpRecipes(onlySkillUp)` | â€” | Filter to skill-up |
| `C_TradeSkillUI.GetOnlyShowSkillUpRecipes()` | `onlySkillUp` | Only skill-up? |

---

## C_CraftingOrders â€” Crafting Orders

### Customer (Placing Orders)

| Function | Returns | Description |
|----------|---------|-------------|
| `C_CraftingOrders.PlaceNewOrder(orderInfo)` | â€” | Place a crafting order |
...
| `C_CraftingOrders.CancelOrder(orderID)` | â€” | Cancel your order |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_CraftingOrders.GetCrafterOrders(request)` | â€” | Query available orders |
| `C_CraftingOrders.GetCrafterBucketTable(request)` | â€” | Get order grouped view |
| `C_CraftingOrders.GetClaimedOrder()` | `order` | Currently claimed order |
| `C_CraftingOrders.ClaimOrder(orderID, professionID)` | â€” | Claim an order |
| `C_CraftingOrders.ReleaseOrder(orderID, professionID)` | â€” | Release claimed order |
| `C_CraftingOrders.FulfillOrder(orderID, crafterNote, professionID)` | â€” | Complete the order |
| `C_CraftingOrders.RejectOrder(orderID, rejectionNote, professionID)` | â€” | Reject the order |
...
| `C_CraftingOrders.OpenCrafterCraftingOrders()` | â€” | Open crafter orders UI |
| `C_CraftingOrders.CloseCrafterCraftingOrders()` | â€” | Close crafter orders UI |
...
## C_ProfessionSpecUI â€” Profession Specialization
...
| `C_ProfessionSpecUI.PurchaseSpecTabPerk(specTabID, perkID)` | â€” | Purchase a perk |
...
| `CastSpell(spellID)` | â€” | Open profession via spell |
...
|-------|---------|-------------|
| `TRADE_SKILL_SHOW` | â€” | Tradeskill UI opened |
| `TRADE_SKILL_CLOSE` | â€” | Tradeskill UI closed |
| `TRADE_SKILL_UPDATE` | â€” | Tradeskill data updated |
| `TRADE_SKILL_LIST_UPDATE` | â€” | Recipe list changed |
| `TRADE_SKILL_DATA_SOURCE_CHANGED` | â€” | Data source changed |
| `TRADE_SKILL_DATA_SOURCE_CHANGING` | â€” | Data source changing |
| `TRADE_SKILL_CRAFT_BEGIN` | â€” | Started crafting |
| `UPDATE_TRADESKILL_CAST_COMPLETE` | â€” | Craft cast completed |
| `UPDATE_TRADESKILL_CAST_STOPPED` | â€” | Craft cast stopped |
...
| `CRAFTINGORDERS_CLAIMED_ORDER_ADDED` | â€” | Order claimed |
| `CRAFTINGORDERS_CLAIMED_ORDER_REMOVED` | â€” | Order released |
| `CRAFTINGORDERS_CLAIMED_ORDER_UPDATED` | â€” | Claimed order updated |
...
| `CRAFTINGORDERS_CUSTOMER_OPTIONS_PARSED` | â€” | Customer options loaded |
| `CRAFTINGORDERS_CRAFTER_ORDER_LIST_UPDATED` | â€” | Crafter order list refreshed |
| `CRAFTINGORDERS_CAN_REQUEST` | â€” | Can request crafter orders |
| `SKILL_LINES_CHANGED` | â€” | Skill lines changed |
...
1. **Tradeskill must be open** â€” Most `C_TradeSkillUI` functions only work when the tradeskill window is open.
2. **CraftRecipe requires hardware event** â€” Crafting requires a user-initiated action (click/key).
3. **Recipe schematic vs recipe info** â€” `GetRecipeSchematic()` provides reagent slots and quality data; `GetRecipeInfo()` provides name/icon/difficulty.
4. **Crafting orders are async** â€” `GetCrafterOrders()` is async. Wait for `CRAFTINGORDERS_CRAFTER_ORDER_LIST_UPDATED`.
5. **Quality tiers** â€” Dragonflight+ recipes have quality tiers (1-5). Use `GetRecipeQualityItemIDs()` to get items per tier.
6. **Recraft** â€” Recrafting uses `RecraftRecipe()` with the item's GUID, not `CraftRecipe()`.
7. **Profession specs use C_Traits** â€” Under the hood, profession specializations use the same `C_Traits` system as class talents with a different config type.
8. **GetProfessions returns indices** â€” Pass the index to `GetProfessionInfo()`, not a profession ID.
