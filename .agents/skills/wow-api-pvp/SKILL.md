...
- **C_PvP** â€” PvP info, rated stats, rewards, queuing
- **Battlegrounds** â€” BG state, score, flags
- **Arenas** â€” Arena info, teams, ratings
- **War Mode** â€” War mode toggle, bonuses
- **Duels** â€” Accept, cancel, start
- **Honor/Conquest** â€” Currency tracking

---

## C_PvP â€” Core PvP System
...
| `C_PvP.ToggleWarMode()` | â€” | Toggle war mode |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_PvP.RequestCrowdControlSpell(unitToken)` | â€” | Request CC info |
| `JoinBattlefield(index [, asGroup [, isRated]])` | â€” | Join BG queue |
| `LeaveBattlefield()` | â€” | Leave BG |
| `AcceptBattlefieldPort(index, acceptFlag)` | â€” | Accept/decline BG port |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `AcceptDuel()` | â€” | Accept duel request |
| `CancelDuel()` | â€” | Decline/cancel duel |
| `StartDuel(unit)` | â€” | Request duel |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_PvP.StartWarGame(targetName, warGameType, ...)` | â€” | Start war game |
...
|-------|---------|-------------|
| `PVP_MATCH_ACTIVE` | â€” | PvP match started |
| `PVP_MATCH_COMPLETE` | winner | Match ended |
| `PVP_MATCH_INACTIVE` | â€” | Match inactive |
| `UPDATE_BATTLEFIELD_STATUS` | battlefieldIndex | Queue status changed |
| `UPDATE_BATTLEFIELD_SCORE` | â€” | Scoreboard updated |
| `BATTLEFIELD_QUEUE_TIMEOUT` | â€” | Queue timed out |
...
| `DUEL_FINISHED` | â€” | Duel ended |
| `DUEL_INBOUNDS` | â€” | Player in duel bounds |
| `DUEL_OUTOFBOUNDS` | â€” | Player out of bounds |
| `HONOR_LEVEL_UPDATE` | isHigherLevel | Honor level changed |
| `PVP_RATED_STATS_UPDATE` | â€” | Rated stats updated |
| `PVP_REWARDS_UPDATE` | â€” | PvP rewards updated |
| `WAR_MODE_STATUS_UPDATE` | isWarModeDesired | War mode toggled |
| `PLAYER_FLAGS_CHANGED` | unitTarget | PvP flag changed |
| `PVP_TIMER_UPDATE` | â€” | PvP timer updated |
| `PVP_VEHICLE_INFO_UPDATED` | â€” | PvP vehicle info changed |
| `PVP_WORLDSTATE_UPDATE` | â€” | World state updated |
| `ARENA_OPPONENT_UPDATE` | unitToken, updateReason | Arena opponent updated |
| `ARENA_PREP_OPPONENT_SPECIALIZATIONS` | â€” | Arena opponent specs |
| `ARENA_SEASON_WORLD_STATE` | â€” | Arena season state |

---

## Gotchas & Restrictions

1. **War mode toggle location** â€” Can only toggle war mode in Stormwind/Orgrimmar (capital cities), not anywhere.
2. **BG score indices** â€” `GetBattlefieldScore()` indices are 1-based and may not match player order.
3. **Queue index** â€” `GetBattlefieldStatus()` uses queue index (1-based), not battlefield ID.
4. **PvP talents** â€” PvP talents are managed through `C_SpecializationInfo`, not `C_PvP`. See the talents skill.
5. **AcceptBattlefieldPort hardware event** â€” Requires a hardware event to accept.
6. **12.0.0 instance restrictions** â€” In BGs and arenas, enemy player info (name, class, health) may be secret values.
7. **Honor is a currency** â€” Honor and conquest are currencies. Use `C_CurrencyInfo` for tracking amounts.
8. **Season resets** â€” Rated PvP data resets each season. `GetCurrentSeasonNumber()` tracks the season.
