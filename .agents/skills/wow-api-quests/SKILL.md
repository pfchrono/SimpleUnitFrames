...
# Quest API (Retail â€” Patch 12.0.0)
...
- **C_QuestLog** â€” Quest log management, tracking, completion, objectives
- **C_QuestInfoSystem** â€” Quest type/tag info, quest classification
- **C_QuestOffer** â€” Quest accept/decline/complete flows
- **C_QuestSession** â€” Party sync quest sessions
- **C_QuestTaskInfo** â€” Bonus objectives, world quests
- **C_ContentTracking** â€” Content tracking system
- **C_CampaignInfo** â€” Campaign (story chapter) tracking
- **C_QuestHubUI** â€” Quest hub display
- **C_QuestLineUI** â€” Quest line (chain) info
- **C_QuestItemUse** â€” Quest item use tracking
- **Quest POI** â€” POI data for quest objectives
- **Quest Gossip** â€” NPC gossip quest interaction

---

## C_QuestLog â€” Quest Log
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_QuestLog.AbandonQuest()` | â€” | Abandon selected quest |
| `C_QuestLog.SetSelectedQuest(questID)` | â€” | Select quest in log |
| `C_QuestLog.GetSelectedQuest()` | `questID` | Currently selected quest |
| `C_QuestLog.SetAbandonQuest(questID)` | â€” | Prepare to abandon |
| `C_QuestLog.RequestLoadQuestByID(questID)` | â€” | Request quest data load |
...
## C_QuestOffer â€” Quest Accept/Decline

| Function | Returns | Description |
|----------|---------|-------------|
| `AcceptQuest()` | â€” | Accept the offered quest |
| `DeclineQuest()` | â€” | Decline the offered quest |
| `CompleteQuest()` | â€” | Complete quest at NPC |
| `GetQuestReward(rewardIndex)` | â€” | Choose reward item |
...
| `C_QuestSession.RequestSessionStart()` | â€” | Start quest session |
| `C_QuestSession.RequestSessionStop()` | â€” | Stop quest session |
| `C_QuestSession.SendSessionBeginResponse(accept)` | â€” | Respond to session invite |
...
| `C_QuestLineUI.RequestQuestLinesForMap(uiMapID)` | â€” | Request quest line data |
...
| `C_ContentTracking.SetTracked(trackableType, trackableID, tracked)` | â€” | Set tracking |
| `C_ContentTracking.StopTracking(trackableType, trackableID)` | â€” | Stop tracking |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_QuestLog.GetQuestPOIs()` | â€” | Request quest POI data |
...
| `QUEST_LOG_UPDATE` | â€” | Quest log changed |
| `QUEST_WATCH_LIST_CHANGED` | questID, added | Tracking changed |
| `QUEST_AUTOCOMPLETE` | questID | Quest auto-completed |
| `QUEST_COMPLETE` | â€” | At NPC to complete quest |
| `QUEST_DETAIL` | questStartItemID | Quest detail shown |
| `QUEST_FINISHED` | â€” | Quest dialog closed |
| `QUEST_GREETING` | â€” | NPC greeting with quests |
| `QUEST_PROGRESS` | â€” | Quest turn-in progress |
| `QUEST_SESSION_CREATED` | â€” | Quest session created |
| `QUEST_SESSION_DESTROYED` | â€” | Quest session ended |
| `QUEST_SESSION_JOINED` | â€” | Joined quest session |
| `QUEST_SESSION_LEFT` | â€” | Left quest session |
| `QUEST_SESSION_MEMBER_CONFIRM` | â€” | Member confirmed session |
| `QUEST_POI_UPDATE` | â€” | Quest POI data changed |
| `TASK_PROGRESS_UPDATE` | â€” | Bonus objective/WQ progress |
...
1. **Quest data may not be loaded** â€” Call `C_QuestLog.RequestLoadQuestByID(questID)` and wait for `QUEST_DATA_LOAD_RESULT` before accessing quest info for quests not in the log.
2. **GetInfo returns headers** â€” Quest log entries include headers (zones). Check `info.isHeader` to filter.
3. **Quest watch limit** â€” There's a maximum number of tracked quests. `AddQuestWatch()` returns false if at limit.
4. **Auto-accept quests** â€” Some quests auto-accept. Check `QuestGetAutoAccept()` to handle gracefully.
5. **World quest vs regular** â€” World quests use `C_TaskQuest` functions, not `C_QuestLog` for map display.
6. **Quest gossip flow** â€” Quest accept flow: `QUEST_GREETING` â†’ `QUEST_DETAIL` â†’ `AcceptQuest()` â†’ `QUEST_ACCEPTED`.
7. **Campaign quests** â€” Use `C_CampaignInfo.IsCampaignQuest()` to identify story quests.
8. **Objective data** â€” `GetQuestObjectives()` returns a table with `text`, `type`, `finished`, `numFulfilled`, `numRequired` per objective.
