...
# Social & Chat API (Retail â€” Patch 12.0.0)
...
- **C_ChatInfo** â€” Chat system, channels, addon messages
- **Chat Frame Functions** â€” SendChatMessage, chat filters, chat bubbles
- **C_Club** â€” Communities / clubs system
- **C_ClubFinder** â€” Club finder / recruitment
- **C_FriendList** â€” Friends list management
- **C_BattleNet** â€” BattleNet friends, presence, game accounts
- **C_VoiceChat** â€” Voice chat system
- **C_SocialRestrictions** â€” Social restriction checks
- **C_SocialQueue** â€” Social queue system
- **C_RecentAllies** â€” Recent allies tracking
- **C_PingManager** â€” In-world ping system
- **C_TTSSettings** â€” Text-to-speech settings

---

## C_ChatInfo â€” Chat System
...
| `C_ChatInfo.ResetDefaultZoneChannels()` | â€” | Reset zone channels |
| `C_ChatInfo.SwapChatChannelsByChannelIndex(i1, i2)` | â€” | Swap channel order |

### Global Chat Functions

| Function | Returns | Description |
|----------|---------|-------------|
| `SendChatMessage(msg [, chatType [, languageID [, target]]])` | â€” | Send chat message |
...
| `LeaveChannelByName(name)` | â€” | Leave a channel |
| `ListChannelByName(name)` | â€” | List channel members |
| `GetChatWindowInfo(frameIndex)` | `name, fontSize, r, g, b, alpha, shown, locked, docked, uninteractable` | Chat window info |
| `SetChatWindowName(frameIndex, name)` | â€” | Set chat window name |
| `LoggingChat(enable)` | â€” | Toggle chat logging |
| `LoggingCombat(enable)` | â€” | Toggle combat logging |

---

## C_Club â€” Communities / Clubs
...
| `C_Club.CreateClub(name, shortName, description, clubType, avatarId)` | â€” | Create new club |
| `C_Club.DestroyClub(clubId)` | â€” | Destroy a club |
| `C_Club.EditClub(clubId, name, shortName, description, avatarId, bgColorIndex)` | â€” | Edit club |
| `C_Club.LeaveClub(clubId)` | â€” | Leave a club |
| `C_Club.AcceptInvitation(clubId)` | â€” | Accept club invitation |
| `C_Club.DeclineInvitation(clubId)` | â€” | Decline club invitation |
| `C_Club.GetInvitationsForSelf()` | `invitations` | Pending invitations |
| `C_Club.SendInvitation(clubId, memberId)` | â€” | Invite to club |
...
| `C_Club.CreateStream(clubId, name, subject, leadersAndModeratorsOnly)` | â€” | Create stream |
| `C_Club.DestroyStream(clubId, streamId)` | â€” | Remove stream |
| `C_Club.EditStream(clubId, streamId, name, subject, leadersAndModeratorsOnly)` | â€” | Edit stream |
...
| `C_Club.SendMessage(clubId, streamId, message)` | â€” | Send message to stream |
| `C_Club.FocusStream(clubId, streamId)` | â€” | Focus a stream |
| `C_Club.UnfocusStream(clubId, streamId)` | â€” | Unfocus a stream |
| `C_Club.SetAutoAdvanceStreamViewMarker(clubId, streamId)` | â€” | Auto-advance view marker |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_ClubFinder.RequestClubsList(type, isLinkedSearch, searchText, specIDs)` | â€” | Search for clubs |
...
| `C_ClubFinder.RequestPostingInformationFromClubId(clubId)` | â€” | Request posting info |
| `C_ClubFinder.PostClub(clubId, minIlvl, minLevel, ...)` | â€” | Post club listing |
| `C_ClubFinder.RequestApplicantList(type)` | â€” | Request applicant list |
| `C_ClubFinder.GetApplicantInfoList()` | `applicants` | Get applicants |
| `C_ClubFinder.RespondToApplicant(clubFinderGUID, playerGUID, accepted, ...)` | â€” | Respond to applicant |

---

## C_FriendList â€” Friends
...
| `C_FriendList.AddFriend(name [, notes])` | â€” | Add friend |
| `C_FriendList.RemoveFriend(name)` | â€” | Remove friend |
| `C_FriendList.SetFriendNotes(name, notes)` | â€” | Set friend notes |
| `C_FriendList.AddOrRemoveFriend(name, notes)` | â€” | Toggle friend |
| `C_FriendList.IsFriend(guid)` | `isFriend` | Is GUID a friend? |
| `C_FriendList.AddIgnore(name)` | â€” | Add to ignore list |
| `C_FriendList.DelIgnore(name)` | â€” | Remove from ignore list |
...
| `C_FriendList.ShowFriends()` | â€” | Refresh friends list |
| `C_FriendList.GetSelectedFriend()` | `index` | Selected friend index |
| `C_FriendList.SetSelectedFriend(index)` | â€” | Select friend |

---

## C_BattleNet â€” BattleNet Friends
...
| `BNSendWhisper(bnetAccountID, message)` | â€” | Send BNet whisper |
| `BNInviteFriend(bnetAccountID)` | â€” | Invite to group |
| `BNRequestInviteFriend(presenceID)` | â€” | Request BNet friend add |
| `BNRemoveFriend(bnetAccountID)` | â€” | Remove BNet friend |
| `BNSetFriendNote(bnetAccountID, note)` | â€” | Set friend note |
| `BNSetCustomMessage(message)` | â€” | Set BNet status message |
| `BNConnected()` | `isConnected` | Connected to BNet? |

---

## C_VoiceChat â€” Voice Chat
...
| `C_VoiceChat.SetMasterVolumeScale(scale)` | â€” | Set master volume |
| `C_VoiceChat.GetInputVolume()` | `volume` | Mic volume |
| `C_VoiceChat.SetInputVolume(volume)` | â€” | Set mic volume |
| `C_VoiceChat.GetOutputVolume()` | `volume` | Speaker volume |
| `C_VoiceChat.SetOutputVolume(volume)` | â€” | Set speaker volume |
| `C_VoiceChat.IsMuted()` | `isMuted` | Is muted? |
| `C_VoiceChat.SetMuted(muted)` | â€” | Set mute |
| `C_VoiceChat.IsDeafened()` | `isDeafened` | Is deafened? |
| `C_VoiceChat.SetDeafened(deafened)` | â€” | Set deafen |
| `C_VoiceChat.IsSilenced()` | `isSilenced` | Is silenced (by Blizzard)? |
| `C_VoiceChat.ActivateChannel(channelID)` | â€” | Activate voice channel |
| `C_VoiceChat.DeactivateChannel(channelID)` | â€” | Deactivate voice channel |
| `C_VoiceChat.IsLoggedIn()` | `isLoggedIn` | Voice chat logged in? |
| `C_VoiceChat.Login()` | â€” | Log in to voice chat |
| `C_VoiceChat.Logout()` | â€” | Log out of voice chat |
...
| `C_VoiceChat.SetMemberVolume(memberID, channelID, volume)` | â€” | Set member volume |
| `C_VoiceChat.IsMemberMuted(memberID, channelID)` | `isMuted` | Is member muted? |
| `C_VoiceChat.SetMemberMuted(memberID, channelID, muted)` | â€” | Mute member |
...
## C_PingManager â€” Ping System

| Function | Returns | Description |
|----------|---------|-------------|
| `C_PingManager.GetPingTypeFromUIMouseButton(button)` | `pingType` | Ping type from mouse button |
| `C_PingManager.TogglePingListener(enabled)` | â€” | Toggle ping listening |

---

## C_TTSSettings â€” Text-to-Speech

| Function | Returns | Description |
|----------|---------|-------------|
| `C_TTSSettings.GetChatTypeEnabled(chatType)` | `enabled` | Is TTS enabled for chat type? |
| `C_TTSSettings.SetChatTypeEnabled(chatType, enabled)` | â€” | Toggle TTS for chat type |
| `C_TTSSettings.GetSetting(setting)` | `value` | Get TTS setting |
| `C_TTSSettings.SetSetting(setting, value)` | â€” | Set TTS setting |
...
| `FRIENDLIST_UPDATE` | â€” | Friends list changed |
| `BN_FRIEND_LIST_SIZE_CHANGED` | â€” | BNet list changed |
| `BN_FRIEND_INFO_CHANGED` | bnetAccountID | BNet friend updated |
| `BN_CONNECTED` | â€” | Connected to BNet |
| `BN_DISCONNECTED` | â€” | Disconnected from BNet |
...
1. **SendAddonMessage BLOCKED in instances** â€” `C_ChatInfo.SendAddonMessage()` and `SendAddonMessageLogged()` throw errors inside dungeons, raids, BGs, and arenas in 12.0.0.
2. **Chat messages may be secret** â€” In instances, chat message content from other players may be KStrings / secret values.
3. **Addon prefix limit** â€” Maximum 16 characters for addon message prefixes.
4. **Addon message size limit** â€” Maximum 255 characters per addon message.
5. **Chat throttle** â€” `SendChatMessage()` is throttled. Sending too fast will disconnect you.
6. **Hardware event required** â€” Some chat functions require a hardware event (mouse click/key press) to execute.
7. **BNet friend list** â€” `BNGetFriendInfo()` is deprecated. Use `C_BattleNet.GetFriendAccountInfo()`.
8. **Club type Guild** â€” Guilds are treated as clubs with `Enum.ClubType.Guild`. Use `C_Club` to interact with guild chat streams.
9. **Voice chat login** â€” Voice chat requires explicit login. Check `C_VoiceChat.IsLoggedIn()` before operations.
10. **Chat filters** â€” `ChatFrame_AddMessageEventFilter` filters operate on ALL instances of the event across all chat frames.
