...
# Spell & Ability API (Retail â€” Patch 12.0.0)
...
- **C_Spell** â€” Core spell information, cooldowns, charges, usability, range, type checks
- **C_SpellBook** â€” Spellbook navigation, skill lines, slot queries, spell lookup
- **C_ActionBar** â€” Action bar slot management, state queries, bar pages, overrides
- **C_SpellActivationOverlay** â€” Spell proc/activation glow detection
- **C_SpellDiminish** â€” Diminishing returns tracking for crowd control
- **C_CooldownViewer** â€” Cooldown viewer categories and layout
- **C_AssistedCombat** â€” Assisted combat (rotation helper) integration
- **Totem** â€” Totem query functions (GetTotemInfo, DestroyTotem)
- **Shapeshifting** â€” Form/stance functions (GetShapeshiftForm, GetShapeshiftFormInfo)
- **Global Spell Functions** â€” CastSpellByName, CastSpellByID, SpellIsTargeting, etc.
...
| Reference | Contents |
|-----------|----------|
| [SPELLS-CORE.md](references/SPELLS-CORE.md) | C_Spell â€” spell info, cooldowns, charges, usability, range, type checks |
| [SPELLBOOK.md](references/SPELLBOOK.md) | C_SpellBook â€” spellbook navigation, skill lines, slot queries |
| [ACTIONBAR.md](references/ACTIONBAR.md) | C_ActionBar â€” action bar slots, state, pages, overrides, pet/vehicle bars |
...
## C_Spell â€” Core Spell API (Quick Reference)

The `C_Spell` namespace provides direct access to spell data by `spellIdentifier` â€” which can be a `spellID` (number) or `spellName` (string).
...
| `C_Spell.GetSpellIDForSpellIdentifier(spellIdentifier)` | `spellID` | Resolves spell name â†’ ID |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_Spell.CancelSpellByID(spellID)` | â€” | Cancel a channeled/cast spell |
| `C_Spell.PickupSpell(spellIdentifier)` | â€” | Attach spell to cursor |
| `C_Spell.RequestLoadSpellData(spellIdentifier)` | â€” | Request async spell data load |
| `C_Spell.IsSpellDataCached(spellIdentifier)` | `isCached` | Is data ready for query? |
| `C_Spell.EnableSpellRangeCheck(spellIdentifier, enable)` | â€” | Enable/disable range checking |
| `C_Spell.SetSpellAutoCastEnabled(spellIdentifier, enabled)` | â€” | Toggle pet spell autocast |
| `C_Spell.ToggleSpellAutoCast(spellIdentifier)` | â€” | Toggle pet spell autocast |
...
## C_SpellBook â€” Spellbook Navigation (Quick Reference)
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_SpellBook.CastSpellBookItem(slot, bank [, targetSelf])` | â€” | Cast from spellbook slot |
| `C_SpellBook.PickupSpellBookItem(slot, bank)` | â€” | Pick up spell to cursor |
| `C_SpellBook.GetSpellBookItemAutoCast(slot, bank)` | `autoCastAllowed, autoCastEnabled` | Autocast state |
| `C_SpellBook.SetSpellBookItemAutoCastEnabled(slot, bank, enabled)` | â€” | Set autocast |
| `C_SpellBook.ToggleSpellBookItemAutoCast(slot, bank)` | â€” | Toggle autocast |
...
## C_ActionBar â€” Action Bar Slot Management (Quick Reference)

The `C_ActionBar` namespace manages the 120+ action bar slots. Actions are referenced by `actionID` (1â€“180) or `slotID`.
...
| Bar | Slots | Notes |
|-----|-------|-------|
| Main bar | 1â€“12 | Always visible |
| Bar 2 (Bottom Left) | 13â€“24 | Toggled via settings |
| Bar 3 (Bottom Right) | 25â€“36 | Toggled via settings |
| Bar 4 (Right) | 37â€“48 | Toggled via settings |
| Bar 5 (Right 2) | 49â€“60 | Toggled via settings |
| Bars 6â€“10 | 61â€“120 | Additional bars |
| StanceBar pages | 121â€“180 | Stance-specific bars |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_ActionBar.GetActionBarPage()` | `currentPage` | Current page (1â€“6) |
| `C_ActionBar.SetActionBarPage(pageIndex)` | â€” | Switch page |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `C_ActionBar.PutActionInSlot(slotID)` | â€” | Place cursor action into slot |
| `C_ActionBar.RegisterActionUIButton(checkboxFrame, actionID, cooldownFrame)` | â€” | Register frame for updates |
| `C_ActionBar.UnregisterActionUIButton(checkboxFrame)` | â€” | Unregister frame |
| `C_ActionBar.ForceUpdateAction(slotID)` | â€” | Force action update |
| `C_ActionBar.EnableActionRangeCheck(actionID, enable)` | â€” | Enable/disable range check |
| `C_ActionBar.ToggleAutoCastPetAction(slotID)` | â€” | Toggle pet autocast |
...
### C_SpellActivationOverlay â€” Spell Procs
...
### C_SpellDiminish â€” Diminishing Returns
...
### C_CooldownViewer â€” Cooldown Display
...
| `C_CooldownViewer.SetLayoutData(data)` | â€” | Save layout state |

> **Wiki:** https://warcraft.wiki.gg/wiki/World_of_Warcraft_API#CooldownViewer

### C_AssistedCombat â€” Rotation Helper
...
### C_LevelLink â€” Spell Level Lock
...
| `DestroyTotem(slot)` | â€” | Destroy totem/minion `#protected` |
| `TargetTotem(slot)` | â€” | Target the totem |
...
| `CancelShapeshiftForm()` | â€” | Leave current form `#protected` |
| `CastShapeshiftForm(index)` | â€” | Enter a form `#protected` |
...
| `SetActionBarToggles(bar1..bar7, alwaysShow)` | â€” | Set visibility of extra bars |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `CastPetAction(index [, target])` | â€” | Cast pet action `#protected` |
...
| `SetMultiCastSpell(actionID, spellID)` | â€” | Set totem bar spell `#protected` |
...
| `CancelUnitBuff(unit, buffIndex [, filter])` | â€” | Remove a buff `#nocombat` |
| `QueryCastSequence(sequence)` | index, item, spell | Preview next step in /castsequence |

---

## Patch 12.0.0 â€” Secret Values Impact
...
- **Cooldown times** â€” `C_Spell.GetSpellCooldown()` may return secret `start`/`duration` when `C_Secrets.ShouldSpellCooldownBeSecret()` is true
- **Spell cast info** â€” `UnitCastingInfo()` / `UnitChannelInfo()` return secret fields when unit identity is restricted
- **Totem info** â€” `GetTotemInfo()` may return secret `startTime`/`duration` when the totem spell is secret
...
local function CanCastOnTarget(spellID, unit)
    ...
    -- Cannot branch on it â€” pass to Cooldown widget instead
    return true
end
```
...
| `ACTIONBAR_PAGE_CHANGED` | â€” | Action bar page changed |
| `ACTIONBAR_UPDATE_COOLDOWN` | â€” | Action cooldown updated |
| `ACTIONBAR_UPDATE_USABLE` | â€” | Action usability changed |
| `ACTIONBAR_UPDATE_STATE` | â€” | Action state changed |
| `UPDATE_BONUS_ACTIONBAR` | â€” | Bonus bar changed (stance/form) |
| `UPDATE_EXTRA_ACTIONBAR` | â€” | Extra action button changed |
| `UPDATE_OVERRIDE_ACTIONBAR` | â€” | Override bar state changed |
| `UPDATE_VEHICLE_ACTIONBAR` | â€” | Vehicle bar changed |
| `UPDATE_POSSESS_BAR` | â€” | Possession bar changed |
...
| `SPELL_UPDATE_COOLDOWN` | â€” | Spell cooldowns changed |
| `SPELL_UPDATE_USABLE` | â€” | Spell usability changed |
| `SPELL_UPDATE_CHARGES` | â€” | Spell charges changed |
| `CURRENT_SPELL_CAST_CHANGED` | â€” | Current cast changed |
| `SPELLS_CHANGED` | â€” | Spellbook contents changed |
...
| `UPDATE_SHAPESHIFT_FORM` | â€” | Shapeshift form changed |
| `UPDATE_SHAPESHIFT_FORMS` | â€” | Available forms changed |
| `UPDATE_SHAPESHIFT_COOLDOWN` | â€” | Form cooldown changed |
...
| `PET_BAR_UPDATE` | â€” | Pet bar updated |
| `PET_BAR_UPDATE_COOLDOWN` | â€” | Pet action cooldown changed |
| `PET_BAR_UPDATE_USABLE` | â€” | Pet action usability changed |
| `ASSISTED_COMBAT_UPDATE` | â€” | Rotation helper suggestion changed |
| `COOLDOWN_VIEWER_UPDATE` | â€” | Cooldown viewer data changed |
