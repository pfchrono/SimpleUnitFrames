...
description: "Complete reference for WoW Retail Class Talents, Hero Talents, Traits system, Specialization, and Profession Specialization APIs. Covers C_ClassTalents (loadouts, config management, talent tree interactions), C_Traits (the underlying trait system â€” nodes, entries, configs, currencies, conditions), C_SpecializationInfo (spec queries, recommended content), global specialization functions (GetSpecialization, GetSpecializationInfo, SetSpecialization), and profession specialization. Use when working with talent trees, talent loadouts, hero talent selection, trait nodes, specialization switching, or talent import/export."
---

# Talents & Specialization API (Retail â€” Patch 12.0.0)
...
- **C_ClassTalents** â€” Talent loadouts, config management
- **C_Traits** â€” Trait system (nodes, entries, configs, currencies)
- **C_SpecializationInfo** â€” Specialization queries
- **Global Spec Functions** â€” GetSpecialization, SetSpecialization
- **Profession Specialization** â€” C_ProfessionSpecUI (cross-ref professions skill)
...
1. **Specialization** â€” Top level: player picks a spec (e.g., Fire Mage)
2. **C_ClassTalents** â€” Manages loadouts (saved talent builds) and provides high-level talent tree interaction
3. **C_Traits** â€” Low-level trait system: nodes, entries, definitions, configs, currencies
4. **Hero Talents** â€” Sub-trees within the talent system, selected as a package

---

## C_ClassTalents â€” Talent Loadouts & Config
...
| `C_ClassTalents.UpdateLastSelectedSavedConfigID(specID, configID)` | â€” | Set last selected config |
...
| `C_ClassTalents.ViewLoadout(loadoutInfo)` | â€” | Preview a loadout |
...
- `ID` â€” Node ID
- `posX`, `posY` â€” Position in tree
- `type` â€” Node type (Single, Tiered, Selection, SubTreeSelection)
- `maxRanks` â€” Maximum ranks
- `activeRank` â€” Current active rank
- `currentRank` â€” Current rank (may differ from active during editing)
- `entryIDs` â€” Available entries (for choice nodes)
- `activeEntry` â€” Currently active entry
- `currentEntry` â€” Entry during editing
- `canPurchaseRank` â€” Can purchase next rank
- `canRefundRank` â€” Can refund a rank
- `isAvailable` â€” Node unlocked
- `isCascadeRepurchasable` â€” Can cascade repurchase
- `isVisible` â€” Visible in UI
- `meetsEdgeRequirements` â€” Edge requirements met
- `groupIDs` â€” Associated groups
- `visibleEdges` â€” Connected edges
- `conditionIDs` â€” Conditions for availability
- `subTreeID` â€” Sub-tree ID (hero talents)
...
- `definitionID` â€” The talent definition
- `type` â€” Entry type
- `maxRanks` â€” Max ranks for this entry
- `isAvailable` â€” Is this entry available
- `conditionIDs` â€” Conditions

### Definition Info Fields

- `spellID` â€” Spell granted by this talent
- `overrideName` â€” Override display name
- `overrideSubtext` â€” Override subtext
- `overrideIcon` â€” Override icon
...
| `SetSpecialization(specIndex)` | â€” | Switch spec |
...
| `C_SpecializationInfo.SetPvpTalentLocked(talentID, locked)` | â€” | Lock PvP talent |
...
| `TRAIT_CONFIG_LIST_UPDATED` | â€” | Config list changed |
...
| `SPECIALIZATION_CHANGE_CAST_FAILED` | â€” | Spec change failed |
| `CONFIRM_TALENT_WIPE` | cost, respecType | Confirm talent wipe |
| `STARTER_BUILD_ACTIVATION_FAILED` | â€” | Starter build failed |
| `SELECTED_PVP_TALENTS_CHANGED` | â€” | PvP talents changed |
| `PVP_TALENTS_UPDATE` | â€” | PvP talents refreshed |
| `HERO_TALENT_SELECTION_CHANGED` | â€” | Hero talent selection changed |

---

## Gotchas & Restrictions

1. **configID is per-spec** â€” Each specialization has its own set of configs. Active configID changes on spec switch.
2. **Cannot change talents in combat** â€” `C_ClassTalents.CommitConfig()` and `SetSpecialization()` fail in combat.
3. **Trait system is generic** â€” `C_Traits` is the underlying system used for class talents, hero talents, AND profession specializations. The `configType` differentiates them.
4. **Node types matter** â€” Single nodes have one entry, Selection (choice) nodes have multiple entries you pick between.
5. **Hero talents are sub-trees** â€” Hero talent selection is a SubTreeSelection node type. Use `C_Traits.GetSubTreeInfo()`.
6. **GetNodeInfo is expensive** â€” Cache node info when iterating the full tree. Don't call in OnUpdate.
7. **Loadout export is opaque** â€” The export string format is internal. Use the API functions, don't parse manually.
8. **specIndex vs specID** â€” `GetSpecialization()` returns an index (1-4), not a specID. Use `GetSpecializationInfo()` to get the specID.
