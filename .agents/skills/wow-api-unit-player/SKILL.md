...
description: "Complete reference for WoW Retail Unit and Player APIs â€” UnitId tokens, GUIDs, health/power/stats, auras, roles, casting, threat, inspection, paper doll, death/resurrection, and player-specific functions. Use when querying unit identity, health, power, auras, stats, combat state, group membership, casting info, threat, or player character details."
---

# Unit & Player API (Retail â€” Patch 12.0.0)
...
- **Unit** â€” Core unit query functions (`UnitName`, `UnitHealth`, `UnitClass`, etc.)
- **UnitAuras** â€” Aura/buff/debuff system (`C_UnitAuras`)
- **UnitRole** â€” Group role assignment and queries
- **PlayerInfo** â€” Player-specific info (`C_PlayerInfo`)
- **PaperDollInfo** â€” Equipment, stats, inspection (`C_PaperDollInfo`)
- **PlayerScript** â€” Global player functions (stats, combat ratings, PvP, death, movement)
- **DeathInfo / IncomingSummon** â€” Death, resurrection, summoning
- **CreatureInfo** â€” Creature data helpers
...
| [UNIT-AURAS.md](references/UNIT-AURAS.md) | C_UnitAuras â€” buffs, debuffs, private auras |
...
## UnitId Tokens â€” Quick Reference
...
|---------|-------|-------------|
| `"partyN"` | 1â€“4 | Nth party member (excluding player) |
| `"raidN"` | 1â€“40 | Nth raid member |
| `"partypetN"` | 1â€“4 | Pet of Nth party member |
| `"raidpetN"` | 1â€“40 | Pet of Nth raid member |
| `"bossN"` | 1â€“8 | Active boss in current encounter |
| `"arenaN"` | 1â€“5 | Opposing arena member |
| `"nameplateN"` | 1â€“40 | Nth nameplate (cannot be targeted) |
...
- `"targettarget"` â€” your target's target
- `"raid7target"` â€” 7th raid member's target
- `"pettarget"` â€” your pet's target
- `"Cogwheel-target"` â€” named player's target (if in group)

> **Source:** https://warcraft.wiki.gg/wiki/UnitId

---

## GUID (Globally Unique Identifier) â€” Quick Reference
...
| Vignette | `Vignette-0-[serverID]-[instanceID]-[zoneUID]-[vignetteID]-[spawnUID]` | â€” |
| BattlePet | `BattlePet-0-[ID]` | `BattlePet-0-00000338F951` |
| Cast | `Cast-[type]-[serverID]-[instanceID]-[zoneUID]-[spellID]-[castUID]` | â€” |
...
## Core Unit Functions â€” Overview
...
|----------|---------|-------|
| `UnitHealth(unit [, usePredicted])` | `health` | **Returns secret value in 12.0** â€” pass directly to widgets |
...
|----------|-------------|
| `C_UnitAuras.AddPrivateAuraAnchor(args)` | Anchor a frame to a private aura's display position â†’ `anchorID` |
| `C_UnitAuras.RemovePrivateAuraAnchor(anchorID)` | Remove private aura anchor |
| `C_UnitAuras.AddPrivateAuraAppliedSound(sound)` | Play sound when private aura applied â†’ `soundID` |
...
-- isFullUpdate (bool) â€” if true, full refresh is needed
...
## C_PlayerInfo â€” Player Information
...
## C_PaperDollInfo â€” Equipment & Inspection
...
| Function | Returns | Description |
|----------|---------|-------------|
| `RepopMe()` | â€” | Release spirit |
| `RetrieveCorpse()` | â€” | Resurrect at corpse |
| `AcceptResurrect()` | â€” | Accept rez offer |
| `DeclineResurrect()` | â€” | Decline rez offer |
...
## Secret Values â€” 12.0.0 Impact on Unit APIs
...
| `PLAYER_TARGET_CHANGED` | â€” | Player target changed |
| `PLAYER_FOCUS_CHANGED` | â€” | Focus target changed |
| `PLAYER_ENTERING_WORLD` | `isInitialLogin, isReloadingUi` | Zone/instance load |
| `PLAYER_DEAD` | â€” | Player died |
| `PLAYER_ALIVE` | â€” | Player alive (accept rez) |
| `PLAYER_UNGHOST` | â€” | Player no longer ghost |
| `PLAYER_REGEN_ENABLED` | â€” | Left combat |
| `PLAYER_REGEN_DISABLED` | â€” | Entered combat |
...
| Secret values (12.0) | Health, name, power may be secret â€” cannot compare/branch, only pass to widgets |
