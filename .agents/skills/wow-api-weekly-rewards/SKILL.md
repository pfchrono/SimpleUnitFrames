...
description: "Complete reference for WoW Retail Weekly Rewards (Great Vault) API. Covers C_WeeklyRewards â€” vault activities (M+, raids, PvP, world), reward selection, item preview, canEquip checks, vault UI state, and related events. Use when working with the Great Vault, weekly reward choices, vault activity tracking, or end-of-week item selection."
---

# Weekly Rewards API (Retail â€” Patch 12.0.0)
...
- **C_WeeklyRewards** â€” Great Vault UI, activities, rewards, item selection

---

## C_WeeklyRewards â€” Great Vault System
...
- `type` â€” Activity type enum
- `index` â€” Slot index (1, 2, 3)
- `threshold` â€” Completions needed
- `progress` â€” Current completions
- `id` â€” Activity ID
- `level` â€” Key level / difficulty / rating tier
- `rewards` â€” Available rewards table
- `rewardOptions` â€” Reward item options

### Reward Selection

| Function | Returns | Description |
|----------|---------|-------------|
| `C_WeeklyRewards.ClaimReward(id)` | â€” | Claim specific reward |
...
|-------|---------|-------------|
| `WEEKLY_REWARDS_UPDATE` | â€” | Vault data changed |
| `WEEKLY_REWARDS_SHOW` | â€” | Vault UI opened |
| `WEEKLY_REWARDS_HIDE` | â€” | Vault UI closed |
| `WEEKLY_REWARDS_ITEM_CHANGED` | â€” | Reward item changed |

---

## Gotchas & Restrictions

1. **Weekly reset timing** â€” Rewards are generated at weekly reset; APIs return no data before Tuesday/Wednesday reset.
2. **Claim requires hardware** â€” `ClaimReward()` requires user interaction.
3. **Activity types vary by season** â€” Not all activity types are active every season.
4. **Three slots per type** â€” Each type has 3 reward slots with increasing thresholds.
5. **Level field meaning changes** â€” For M+ it's key level, for raids it's difficulty, for PvP it's rating bracket.
6. **Example rewards are estimates** â€” `GetExampleRewardItemHyperlinks()` shows possible rewards, not guaranteed ones.
7. **Rewards expire** â€” Unclaimed vault rewards from a previous week may be forfeited.
8. **HasAvailableRewards() vs HasGeneratedRewards()** â€” Generated means the vault has rolled items; Available means the player can claim them now.
