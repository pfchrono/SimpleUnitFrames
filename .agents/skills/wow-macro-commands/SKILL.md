...
A macro is a list of slash commands executed sequentially from an action bar button. Each command goes on its own line, preceded by `/`. Macros have a **255 character limit**. Commands are **not** case-sensitive. Multiple commands run in a single frame â€” there is no way to wait or delay between lines.
...
- All commands in a macro execute in the **same frame** â€” no delays.
...
| `/dismount` | Dismount |
| `/script` (or `/run`) | Execute Lua code (insecure â€” cannot cast spells) |
...
Temporarily target a unit for this command only â€” does NOT change your selected target:
- `@player`, `@target`, `@focus`, `@mouseover`, `@party1`, `@raid5`, etc.
- `@cursor` â€” immediately targets ground under cursor
- `@none` â€” interrupts auto self-cast, requires targeting cursor
...
```
- `reset=N` â€” reset after N seconds of inactivity
- `reset=target` â€” reset on target change
- `reset=combat` â€” reset on leaving combat
- `reset=shift/alt/ctrl` â€” reset when modifier held
...
```
- `#show` â€” controls icon only
- `#showtooltip` â€” controls icon AND tooltip
