...
local function FormatTimeCompat(seconds, short)
	if type(oUF.GetTime) == 'function' then
		return oUF:GetTime(seconds, short)
	end
...
local function UnpackAuraDataCompat(aura)
	...
	local isBossDebuff = aura.isBossAura
	local castByPlayer = aura.isFromPlayerOrPlayerPet
	local nameplateShowAll = aura.nameplateShowAll
	local modRate = aura.timeMod
	local effect1, effect2, effect3

	return name, texture, count, debuffType, duration, expiration, source, isStealable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff, castByPlayer, nameplateShowAll, modRate, effect1, effect2, effect3
end
...
local function OnEnter(self)
	...
	if type(oUF.SetTooltipByAuraInstanceID) == 'function' then
...
local function UpdateValue(bar, start)
	if oUF.isRetail then
		...
		bar.timeText:SetFormattedText(FormatTimeCompat(remain))
	end
end
...
local function UpdateBar(element, bar)
	if IsSecretValueCompat(bar.count) then
		if bar.aura then
	...
	if bar.filter == 'HARMFUL' then
		if IsSecretValueCompat(debuffType) or not debuffType or debuffType == '' then
			debuffType = 'None'
		end
...
local function AuraUpdate(element, unit, aura, index, offset, filter, isDebuff, visible)
	local name, texture, count, debuffType, duration, expiration, source, isStealable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff, castByPlayer, nameplateShowAll, modRate, effect1, effect2, effect3 = UnpackAuraDataCompat(aura)
	...
	bar.isPlayer = NotSecretValueCompat(source) and (source == 'player' or source == 'vehicle') or (aura and aura.auraIsPlayer) or nil
	...
	bar.noTime = NotSecretValueCompat(duration) and (duration == 0 and expiration == 0)
...
local function FilterBars(element, unit, filter, limit, isDebuff, offset, dontHide)
	...
	elseif C_UnitAuras and C_UnitAuras.GetAuraSlots and C_UnitAuras.GetAuraDataBySlot then
		...
		repeat
			local slots = {C_UnitAuras.GetAuraSlots(unit, filter, limit - visible, continuationToken)}
		...
		until not continuationToken or visible >= limit
	end
