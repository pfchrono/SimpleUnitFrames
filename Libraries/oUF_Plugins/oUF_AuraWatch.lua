...
local function UnpackAuraDataCompat(aura)
	...
	return name, icon, count, debuffType, duration, expiration, source, isStealable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff, castByPlayer, nameplateShowAll, modRate, effect1, effect2, effect3
end
...
local function CustomFilter(element, _, button, _, _, _, count)
	...
	local setting = NotSecretValueCompat(spellID) and element.watched[spellID]
	if not setting then
		return false
	end
...
local function UpdateIcon(element, unit, aura, index, offset, filter, isDebuff, visible)
	local name, icon, count, debuffType, duration, expiration, source, isStealable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff, castByPlayer, nameplateShowAll, modRate, effect1, effect2, effect3 = UnpackAuraDataCompat(aura)
	...
	button.isPlayer = (NotSecretValueCompat(source) and source == 'player') or (aura and aura.auraIsPlayer) or nil
