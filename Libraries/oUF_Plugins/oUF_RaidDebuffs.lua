...
local function GetSpellInfoCompat(spellID)
	if oUF and oUF.GetSpellInfo then
		local name, rank, icon = oUF:GetSpellInfo(spellID)
		if name then
...
local function AddSpell(spell, priority, stackThreshold)
	if addon.MatchBySpellName and type(spell) == 'number' then
		spell = GetSpellInfoCompat(spell)
	end
...
local function OnUpdate(self, elapsed)
	...
	if self.elapsed >= 0.1 then
		...
		if timeLeft > 0 then
			self.time:SetFormattedText(FormatTimeCompat(timeLeft, true))
		else
...
local function UpdateDebuff(element, aura, forced, stackThreshold)
	...
	if forced then
		...
		name, _, icon = GetSpellInfoCompat(spellID)
	elseif aura then
...
local function Update(self, event, unit, updateInfo)
	...
	if not _forced then
		...
		local function HandleAura(aura)
			local debuffType = NotSecretValueCompat(aura.dispelName) and aura.dispelName -- we coudln't dispel if the unit its charmed, or its not friendly
			...
			local data = (NotSecretValueCompat(aura.spellId) and NotSecretValueCompat(aura.name)) and (debuff_data[aura.spellId] or (not element.onlyMatchSpellID and debuff_data[aura.name]))
		...
		if AuraFiltered and AuraFiltered.HARMFUL and AuraFiltered.HARMFUL[unit] then
