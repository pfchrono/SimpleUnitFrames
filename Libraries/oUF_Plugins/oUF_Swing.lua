...
local function GetSpellNameCompat(spellID)
	if oUF and oUF.GetSpellInfo then
		local name = oUF:GetSpellInfo(spellID)
...
do
	...
	local slam = GetSpellNameCompat(1464)
...
local Ranged = function(self, event, unit, spellName)
	local autoShot = GetSpellNameCompat(75)
	local shoot = GetSpellNameCompat(5019)
	if unit ~= 'player' or (spellName ~= autoShot and spellName ~= shoot) then return end
