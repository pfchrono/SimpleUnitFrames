		...
		sections = {
			...
			},
			{
				...
				controls = {
					...
					{
						type = "paragraph",
						getText = function() return "|cff99ff00Indicator Glows|r" end,
					},
					{
						type = "check",
						label = "Party Leader Indicator Glow",
						get = function() return addon.db.profile.visibility.enablePartyLeaderGlow ~= false end,
						set = function(v) addon.db.profile.visibility.enablePartyLeaderGlow = v and true or false; addon:RefreshAllGlowIndicators() end,
					},
					{
						type = "check",
						label = "Threat Indicator Glow",
						get = function() return addon.db.profile.visibility.enableThreatIndicatorGlow ~= false end,
						set = function(v) addon.db.profile.visibility.enableThreatIndicatorGlow = v and true or false; addon:RefreshAllGlowIndicators() end,
					},
					{
						type = "check",
						label = "Target/Selected Unit Glow",
						get = function() return addon.db.profile.visibility.enableTargetSelectionGlow ~= false end,
						set = function(v) addon.db.profile.visibility.enableTargetSelectionGlow = v and true or false; addon:RefreshAllGlowIndicators() end,
					},
				},
			},
			{
			...
			},
			{
				...
				controls = {
					...
					},
					{
						...
						step = 1,
						format = "%.0f",
						get = function() return tonumber(addon.db.profile.party.spacing) or 15 end,
						set = function(v) addon.db.profile.party.spacing = math.floor(v + 0.5); addon:ApplyPartyHeaderSettings() end,
					},
					{
						type = "slider",
						label = "Boss Vertical Spacing",
						min = 10,
						max = 50,
						step = 1,
						format = "%.0f",
						get = function() return tonumber(addon.db.profile.boss and addon.db.profile.boss.spacing) or 15 end,
						set = function(v) if not addon.db.profile.boss then addon.db.profile.boss = {} end; addon.db.profile.boss.spacing = math.floor(v + 0.5); addon:TrySpawnGroupHeaders() end,
					},
				},
			},
			{
